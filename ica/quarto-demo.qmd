...
`*` *test* puts text in italics

`**` **test** puts text in bold

`#` leaves a comment in an r chunk 

`-` puts line across page, to help separate the headers

`\` leaves line blank

`![](url)` can put in a link
...
in both the .qmd and the .html it just appears as text rather than running the code
...
- Press the green arrow in the top right of the chunk. What happens in the qmd? the function works and the result of the code appears in the qmd
- Render. What appears in the html: R code, output, or both? both!!
...
rep(x = 1, times = 10)
seq(from=100, to=1000, length=20)
number_students_in_class <- 27
number_students_in_class
```
